...
def add_to_cart_view(request, pk):
    ...
    response = render(request, 'ecom/index.html', {'products': products})

    # Mettre à jour les cookies
    response.set_cookie('product_ids', product_ids)
    messages.info(request, f"{product.name} ajouté au panier avec succès!")
    return response
...
def customer_address_view(request):
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            # Supprimer les cookies après la commande
            response = redirect('my_order')  # Rediriger vers la page "Mes commandes"
            response.delete_cookie('product_ids')
